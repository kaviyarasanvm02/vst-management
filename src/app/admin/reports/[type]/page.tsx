...
export default async function ReportsPage({
    ...
    if (!['attendance', 'employee', 'project'].includes(type)) {
        notFound();
    }
                ...
                <div className="space-y-5">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <div className="h-12 w-12 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-lg">
                                    {(report as any).employee.name.charAt(0).toUpperCase()}
                                </div>
                                <div>
                                    <p className="font-bold text-slate-900">{(report as any).employee.name}</p>
                                    <p className="text-xs text-slate-500">{(report as any).employee.email} · {(report as any).employee.role} · {(report as any).employee.branch}</p>
                                </div>
                            </div>
                            <p className="text-sm font-semibold text-slate-500">{monthLabel} {(report as any).year}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-5 pt-5 border-t border-slate-100">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-slate-900">{(report as any).totalHours}h</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-slate-900">{(report as any).daysWorked}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-slate-900">{(report as any).byProject.length}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {(report as any).byProject.map((p: any) => {
                                    const pct = (report as any).totalHours > 0 ? Math.round((p.hours / (report as any).totalHours) * 100) : 0;
                                    return (
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100">
                            <h2 className="text-sm font-bold text-slate-900">Daily Entries ({(report as any).entries.length})</h2>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead className="bg-slate-50 text-xs text-slate-500 uppercase tracking-wide">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {(report as any).entries.map((e: any, i: number) => (
                                ...
                                <tfoot className="bg-slate-50 border-t border-slate-200">
                                    <tr>
                                        ...
                                        <td className="px-5 py-3 text-right font-extrabold text-indigo-700">{(report as any).totalHours}h</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
