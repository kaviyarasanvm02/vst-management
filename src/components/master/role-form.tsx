...
import { createRole } from '@/app/lib/master-actions';
...
function SubmitButton() {
    ...
    return (
        <button type="submit" disabled={pending}
            ...
            {pending ? <><span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" /> Saving...</> : 'Save Role'}
        </button>
    );
}
...
export default function RoleForm() {
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h2 className="text-base font-semibold text-slate-900 mb-4">Add New Role</h2>
            <form action={formAction} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className={labelClass}>Code *</label><input name="code" required className={inputClass} placeholder="e.g., DEV" /></div>
                    <div><label className={labelClass}>Name *</label><input name="name" required className={inputClass} placeholder="e.g., Developer" /></div>
                    <div className="md:col-span-2"><label className={labelClass}>Remarks</label><input name="remarks" className={inputClass} placeholder="Optional notes" /></div>
                </div>
